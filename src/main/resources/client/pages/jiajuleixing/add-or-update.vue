	...
	<view :style='{"width":"100%","padding":"0 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"24rpx 24rpx 24rpx 24rpx","background":"#ffffff","display":"block","height":"auto"}' class="app-update-pv">
			<view :style='{"padding":"4rpx 0 4rpx 0","margin":"0 0 24rpx 0","borderColor":"#eeeeee","alignItems":"center","borderWidth":"0 0 2rpx 0","background":"#ffffff","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#333333","textAlign":"right"}' class="title">家具类型</view>
				<input :style='{"border":"0px solid #eeeeee","padding":"0px 24rpx 0px 24rpx","margin":"0 0 0 0","color":"#666666","borderRadius":"8rpx","flex":"1","background":"#ffffff00","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx"}' :disabled="ro.jiajuleixing" v-model="ruleForm.jiajuleixing" placeholder="家具类型"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`jiajuleixing`, this.ruleForm.id);
			...
			if(options.cross){
				...
				for (var o in obj){
					if(o=='jiajuleixing'){
					this.ruleForm.jiajuleixing = obj[o];
					this.ro.jiajuleixing = true;
					continue;
					}
				}
			}
...
//跨表计算判断
				var obj;
				if((!this.ruleForm.jiajuleixing)){
					this.$utils.msg(`家具类型不能为空`);
					return
				}
				//更新跨表属性
			       var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`jiajuleixing`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`jiajuleixing`, this.ruleForm);
						}else{
							await this.$api.add(`jiajuleixing`, this.ruleForm);
						}
