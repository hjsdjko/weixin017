	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('jiajuleixing', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','jiajuleixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','jiajuleixing');
			...
			async init(){
				...
				let res = await this.$api.info('jiajuleixing', this.id);
				this.detail = res.data;
			},
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussjiajuleixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
